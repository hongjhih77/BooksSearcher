            ...
            if(node instanceof Element ){
              for(Node node_author : node.childNodes()){
                if (node_author instanceof TextNode){
                  ...
                  if (author.length() > 0) authors.add(((TextNode) node_author).getWholeText().trim());
                }
              }
            }
          }
          ...
          for( Element li : lis ){
            if (li.toString().contains("規格")){
              for(Node node : li.childNodes()){
                if (node instanceof TextNode){
                  String pageString = ((TextNode) node).getWholeText().trim().split("：")[1].split("/")[1].trim().replace("頁", "").trim();
